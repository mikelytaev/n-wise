...
### Создание n-wise
Создание начинается с инициализации реестра состояний. Данный RFC НЕ УСТАНАВЛИВАЕТ порядок создания реестра состояний. После создания реестра состояний, создатель n-wise записывает в него транзакцию `genesisTx`. Создатель автоматически считается администратором n-wise. Создатель ДОЛЖЕН сгенерировать новый уникальный DID и DID Document для n-wise.

### Приглашение участника
Любой участник с ролью администратора может создать приглашение на присоединение к n-wise. Сначала Inviter генерирует пару публичного и приватного ключа приглашения согласно Ed25519. Публичный ключ приглашения записывается в реестр состояния при помощи транзакции типа `invitationTx`.
