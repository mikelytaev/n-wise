...
Транзакция `invitationTx` ДОЛЖНА быть подписана публичным ключом user, определенном в его DID Document.
...
Транзакция `updateParticipantTx` ДОЛЖНА быть подписана публичным ключом пользователя, информация о котором обновляется. Указанный публичный ключ ДОЛЖЕН быть определен в предыдущей версии DID Document.
